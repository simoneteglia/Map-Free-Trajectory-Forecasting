...
class TrajectoriesDatasetGenerator(Dataset):
    def __init__(self, dataframe, history_len, future_len, tot_analyzed_frames):
    ...
    def __getitem__(self, index):
        ...
        targets = features[self.history_len:,:,:] # (future_len, num_vehicles, vehicle_features)
...
def main(mode, root_path, history_len, future_len, tot_analyzed_frames, predicted_features):
    ...
    if (mode == 'create_data'):
        ...
        df = pd.read_csv(root_path+"Dataset/sind_smoothed_tracks.csv")
        
        dataset = TrajectoriesDatasetGenerator(df, history_len, future_len, tot_analyzed_frames)
        ...
        np.save(root_path+"Dataset/Traj_Inputs",inputs)
        np.save(root_path+"Dataset/Traj_Targets",targets)
        np.save(root_path+"Dataset/Traj_Inputs_Masks",inputs_masks)
        np.save(root_path+"Dataset/Traj_targets_Masks",targets_masks)
        np.save(root_path+"Dataset/Max_Num_Vehicles",max_num_vehicles)

    # AVVIA TRAINING
    elif (mode == 'load_data'):
       
        inputs = np.load(root_path+"Dataset/Traj_Inputs.npy", allow_pickle=True)
        targets = np.load(root_path+"Dataset/Traj_Targets.npy", allow_pickle=True)
        inputs_masks = np.load(root_path+"Dataset/Traj_Inputs_Masks.npy", allow_pickle=True)
        targets_masks = np.load(root_path+"Dataset/Traj_targets_Masks.npy", allow_pickle=True)
        max_num_vehicles = np.load(root_path+"Dataset/Max_Num_Vehicles.npy", allow_pickle=True)
    ...
    data_module = TrajectoryDataModule(train_data, val_data, test_data, batch_size=64)
